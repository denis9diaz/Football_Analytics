    ...
    def handle(self, *args, **kwargs):
        ...
        for partido in partidos_hoy:
            ...
            def calcular_prob(equipo, fecha_limite):
                ...
                self.stdout.write(f"\n🔎 {equipo} - partidos analizados (Over 2.5):")
                cumple = []
                for p in historial:
                    goles = p.goles_local_ft + p.goles_visitante_ft
                    cumple.append(goles > 2)
                    self.stdout.write(f"  • {p.fecha} {p.equipo_local} vs {p.equipo_visitante} → Goles: {goles} → {'✅' if goles > 2 else '❌'}")
                ...
                self.stdout.write(f"👉 % partidos Over 2.5: {round(porcentaje*100,2)}%, racha sin cumplir: {racha}, prob final: {round(prob*100,2)}%\n")
                return prob
